...
describe('LearnIntervals', () => {
  it('renders 12 note buttons in the strip', () => {
    ...
    expect(wrapper.findAll('.tc-note-strip .tc-note-strip-pill')).toHaveLength(12)
  })

  it('shows the interval reference grid with 11 entries', () => {
    const wrapper = mount(LearnIntervals)
    expect(wrapper.findAll('.tc-learn-intervals-ref-item')).toHaveLength(11)
  })

  it('clicking a ref interval highlights it and shows interval info', async () => {
    const wrapper = mount(LearnIntervals)
    await wrapper.findAll('.tc-learn-intervals-ref-item')[6].trigger('click')
    expect(wrapper.findAll('.tc-learn-intervals-ref-item')[6].classes()).toContain('highlight')
    expect(wrapper.find('.tc-learn-intervals-iv-name').text()).toBe('Perfect 5th')
  })

  it('clicking a ref interval calls playChord with correct notes', async () => {
    const wrapper = mount(LearnIntervals)
    await wrapper.findAll('.tc-learn-intervals-ref-item')[0].trigger('click')
  ...
  it('clicking the same ref interval again deselects it', async () => {
    ...
    await wrapper.findAll('.tc-learn-intervals-ref-item')[0].trigger('click')
    await wrapper.findAll('.tc-learn-intervals-ref-item')[0].trigger('click')
    expect(wrapper.findAll('.tc-learn-intervals-ref-item')[0].classes()).not.toContain('highlight')
    expect(wrapper.find('.tc-learn-intervals-iv-name').exists()).toBe(false)
  })

  it('picks first note on first tap and sets from class', async () => {
    const wrapper = mount(LearnIntervals)
    const pills = wrapper.findAll('.tc-note-strip-pill')
  ...
  it('shows intervalInfo after picking two different notes', async () => {
    ...
    expect(wrapper.find('.tc-learn-intervals-iv-name').text()).toBe('Major 3rd')
    expect(wrapper.find('.tc-learn-intervals-iv-path').text()).toBe('C → E')
  })
  ...
  it('shows intervalInfo for minor 3rd', async () => {
    ...
    expect(wrapper.find('.tc-learn-intervals-iv-name').text()).toBe('Minor 3rd')
  })
  ...
  it('tapping same note again resets from and starts over', async () => {
    ...
    expect(wrapper.find('.tc-learn-intervals-iv-name').exists()).toBe(false)
    expect(wrapper.find('.tc-learn-intervals-iv-hint').text()).toBe('Now pick a second note')
  })
})
