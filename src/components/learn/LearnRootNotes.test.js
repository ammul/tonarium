...
describe('LearnRootNotes', () => {
  it('renders 12 note buttons', () => {
    ...
    expect(wrapper.findAll('.tc-note-strip-pill')).toHaveLength(12)
  })

  it('shows hint text when no note is selected', () => {
    const wrapper = mount(LearnRootNotes)
    expect(wrapper.find('.tc-learn-roots-rr-hint').exists()).toBe(true)
  })
  ...
  it('calls playNote with the correct MIDI note', async () => {
    const wrapper = mount(LearnRootNotes)
    await wrapper.findAll('.tc-note-strip-pill')[2].trigger('pointerdown') // B (index 2, A-based)
    expect(playNote).toHaveBeenCalledWith(71) // 60 + NOTE_TO_SEMI[2] = 60 + 11
  })

  it('selected note gets the from class', async () => {
    const wrapper = mount(LearnRootNotes)
    await wrapper.findAll('.tc-note-strip-pill')[5].trigger('pointerdown') // F
    expect(wrapper.findAll('.tc-note-strip-pill')[5].classes()).toContain('from')
    expect(wrapper.findAll('.tc-note-strip-pill')[0].classes()).not.toContain('from')
  })
  ...
  it('shows the octave button after picking a note', async () => {
    const wrapper = mount(LearnRootNotes)
    await wrapper.findAll('.tc-note-strip-pill')[0].trigger('pointerdown')
    expect(wrapper.find('.tc-learn-roots-rr-octave-btn').exists()).toBe(true)
  })
})
