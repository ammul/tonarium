...
describe('LearnScales', () => {
  it('renders scale selector tabs', () => {
    ...
    expect(wrapper.findAll('.tc-learn-scales-tab').length).toBeGreaterThan(0)
  })

  it('renders scale display tiles for all 12 chromatic notes', () => {
    const wrapper = mount(LearnScales)
    expect(wrapper.findAll('.tc-learn-scales-tile')).toHaveLength(12)
  })

  it('Major scale highlights 7 notes', () => {
    const wrapper = mount(LearnScales)
    expect(wrapper.findAll('.tc-learn-scales-tile.active').length).toBe(7)
  })

  it('C major scale has root class on C tile', () => {
    const wrapper = mount(LearnScales)
    const rootTiles = wrapper.findAll('.tc-learn-scales-tile.root')
