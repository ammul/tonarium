...
describe('ProgressionBuilder', () => {
  ...
  it('renders the text input', () => {
    const w = mount(ProgressionBuilder)
    expect(w.find('input.tc-prog-builder-chord-input').exists()).toBe(true)
  })

  it('parses valid major chord', async () => {
    const w = wrapper('C')
    await w.vm.$nextTick()
    expect(w.findAll('.tc-prog-builder-chord-card').length).toBe(1)
    expect(w.find('.tc-prog-builder-chord-name').text()).toBe('C')
  })

  it('parses minor chord', async () => {
    const w = wrapper('Am')
    await w.vm.$nextTick()
    expect(w.find('.tc-prog-builder-chord-name').text()).toBe('Am')
  })

  it('parses dominant 7th', async () => {
    const w = wrapper('G7')
    await w.vm.$nextTick()
    expect(w.find('.tc-prog-builder-chord-name').text()).toBe('G7')
  })

  it('parses sharp notes', async () => {
    const w = wrapper('F#m')
    await w.vm.$nextTick()
    expect(w.find('.tc-prog-builder-chord-name').text()).toBe('F#m')
  })

  it('converts flats to sharps', async () => {
    const w = wrapper('Bb')
    await w.vm.$nextTick()
    expect(w.find('.tc-prog-builder-chord-name').text()).toBe('A#')
  })

  it('shows error badge for invalid tokens', async () => {
    const w = wrapper('C xyz G')
    await w.vm.$nextTick()
    expect(w.find('.tc-prog-builder-error-token').exists()).toBe(true)
    expect(w.find('.tc-prog-builder-error-token').text()).toBe('xyz')
  })

  it('parses multiple valid chords', async () => {
    const w = wrapper('D f#m E D')
    await w.vm.$nextTick()
    expect(w.findAll('.tc-prog-builder-chord-card').length).toBe(4)
  })
})
